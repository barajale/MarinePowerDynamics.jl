...
# based on data Source: Kodsi, S. K. M., & Canizares, C. A. (2003). Modeling and simulation of IEEE 14-bus system with FACTS controllers. University of Waterloo, Canada, Tech. Rep.

# added bus 15 as a PTO and branch 21 as a connection to bus 6
...
function build_grids()
    ...
    write_powergrid(powergrid, joinpath(@__DIR__,"./IEEE14grid.json"), Json)
    ...
    buses=OrderedDict(
        ...
        "bus13"=> PQAlgebraic(P=-0.135, Q=-0.058),
        "bus14"=> PQAlgebraic(P=-0.149, Q=-0.05),
        "bus15" => LinearPTO(τ_P=0.1,τ_Q=2.0,K_P=0.1,K_Q=0.1,V_r=1.0,Q=0,K_pto=0, C_pto=1200000, η=1.0, base_power=100e6, scaling_factor=2.0, sim_path="./src/RM3_data/simulation_data.csv")
        );
    ...
    powergrid = PowerGrid(buses, branches)
    write_powergrid(powergrid, joinpath(@__DIR__, "./WEC_IEEE14grid.json"), Json) 
end